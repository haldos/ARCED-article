...
attempts to use 2-D masks for this purpose. These operators are based on computing the diagonal \tcom{differences} \myn implemented by filtering an image with the masks in Figure \ref{fig:roberts}. By convention 
...
\myn{\label{correction_a:first_derivative:sobel} The left mask can be constructed (beyond normalization) as the convolution of a vertical derivative filter
$[-1\ 0\ 1]$ with an horizontally moving average filter $[1\ 1\ 1]$. The same reasoning could be applied to the right mask. Hence, these operators can be interpreted as the consecutive application of an horizontal low pass filter with a vertical derivative filter. Thus, they show a smoothing behaviour in the direction orthogonal to the gradient.
...
should be zero at $\rho=0$, but this condition is relaxed, allowing the the polynomial becomes zero in a neighborhood 
