...
such as Sobel, Prewitt and Roberts. In the second reviewed family it is used second derivative, for example in algorithms 
...
\begin{ipolCode}
For all the algorithms reviewed, an open source C implementation is provided and can be downloaded from 
...
proposing a model to approximate locally the image around a point. Then, using this model, 
...
All the algorithms based on first derivatives that are studied in this paper share a common structure, the only difference being the type of filtering used to compute those derivatives. Figure \ref{fig:blockdiagram1} presents a block diagram of those algorithms. \myn{\label{correction_a:first_derivative:first_deriv_rephrase}\\
...
attempts to use 2-D masks for this purpose. These operators are based on computing the diagonal \tcom{differences} \myn{\label{correction_a:typo}} implemented by filtering an image with the masks in Figure \ref{fig:roberts}. By convention 
...
by taking the difference between the third and first rows (or columns) of the $3\times3$ region. The difference between the third and first rows approximates the derivative in the x-direction, and the difference between the third and first columns approximates the derivative in the y-direction.
These approximations can be implemented by filtering the image with the two masks shown in Figure \ref{fig:prewitt}. Those are called \textit{Prewitt operators}. \\
...
\myn{\label{correction_a:first_derivative:sobel}} The left mask can be constructed (up to the normalization factor) as the convolution of two filters: a vertical derivative 
$[-1\ 0\ 1]$ and an horizontal moving average $[1\ 1\ 1]$. The same reasoning could be applied to the right mask. Hence, these operators can be interpreted as the consecutive application of an horizontal low pass filter with a vertical derivative filter. Thus, they show a smoothing behaviour in the direction orthogonal to the gradient.

%-------------------------------------------------------------------------------

\subsection{The \textit{Sobel} operators}

\textit{Sobel operators} are a slight variation of the Prewitt operators which use more weight on the central coefficients of the 
difference. It can be shown that using a $2$ in the center location provides better image smoothing. This
variation is implemented using masks in Figure \ref{fig:sobel}. 
...
Sobel masks can be seen (up to the normalization factor) as the convolution of a horizontal derivation mask 
...
use Sobel operators, because they are better localized, and their filter are also less aliased, because of the weighted shape of $[1\ 2\ 1]$.
...
\begin{equation*}
	\mbox{edges}[i] = \begin{cases} 0,& \mbox{if}\ M[i]\geq\mbox{th} \\
									1,& \mbox{if}\ M[i]<\mbox{th}
					  \end{cases}
\end{equation*}
...
\label{algo:mh}
\begin{algorithmic}[1]
\REQUIRE input image, standard deviation $\sigma$, kernel size $n$ and zero-crossing threshold $th_{ZC}$.
\STATE $im_{ORIG} \leftarrow$ input image
\IF{$im_{ORIG}$ is grayscale}
	\STATE $im \leftarrow im_{ORIG}$
\ELSE 
...
\ELSE
	\STATE $kernel \leftarrow$ generate\_kernel($n$,$\sigma$) \COMMENT{Generated Gaussian or LoG kernel}\mynf{\label{correction_a:second_derivative:marr_algo}}.
...
to find 10 coefficients. As mentioned before this leads to an \tcom{overdetermined} system which can be solved by least squares. 
...
\myn{\label{second_derivative:haralick:analitical:statement}} After the neighborhood of each point of the image is approximated using the bi-cubic polynomial expression in equation \ref{eq:bicubic}, the idea of Haralick's algorithm is to compute the second derivative of the image in the direction of the gradient using that approximation.\\

Let $\theta$ be the gradient angle, defined with respect to the negative x-axis, and it is \tcom{approximated} using the first order terms of the 
polynomial \ref{eq:bicubic} obtaining the following expression:
...
\myn{\label{second_derivative:haralick:analitical:derivative}} Let us consider the (half) line $r$ passing by $(x,y)$ with angle $\theta$. Each point $(u,v)$ on this line can be described in terms of  the angle $\theta$ and the distance $\rho$ to $(x,y)$ as follows
...
\end{equation}
 ...
 Ideally, to hold this condition, the polynomial function 
should be zero at $\rho=0$, but this condition is relaxed, allowing the polynomial becomes zero in a neighborhood 
of $\rho=0$ ($\rho\leq\rho_0$). \\
...
where $\rho_0$ is a fixed threshold, which passed as an argument to the algorithm.
\myn{\label{second_derivative:haralick:analitical:parameter_value}}  This parameter must be greater than zero and and less than $\sqrt{2}$. Its optimal  value is between $0.4$ and $0.6$.\\
...
\ELSE 
	\STATE $im \leftarrow (6968im_{ORIG,R}+23434im_{ORIG,G}+2366im_{ORIG,B})/32768$ \COMMENT{Grayscale conversion, assuming RGB image.}
\ENDIF
\STATE Define the ten $5x5$ masks ($mask[1] \dots mask[10]$) used to determine coefficients $k_1$ to $k_{10}$ \COMMENT{See Table \ref{table:b_i}.} \mynf{\label{correction_a:second_derivative:haralick_algo}}
...
relative to the edge of the image. Direct convolution is not the only way of filtering an image with a kernel. There are other methods such as FFT convolution 
...
which were used to generate the detailed code description presented in section \ref{sec:appendix2}. We think this tool could be useful for future IPOL articles. 
