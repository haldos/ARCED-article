%-------------------------------------------------------------------------------
% A Review of Classic Edge Detectors - v12
...
variation is implemented using the masks in Figure \ref{fig:sobel}. These operators are called 
...
The computational difference between Prewitt and Sobel masks is not important, thus It is preferable to 
use Sobel operators, because edges are better localized, and their filter are also less aliased, 
...
\end{equation}
where\footnote{In this matrix the coefficients $b_i$ are simply spatially arranged with the same topology as the pixels they multiply.}
\begin{equation}
	\label{eq:b_i}
...
should be zero at $\rho=0$, but this condition is relaxed\footnote{This polynomial being zero at $\rho = 0$ means that the edge passes right through the exact coordinates of the pixel. Since the edge may be slightly displaced, other $\rho$ values are permitted as long as the edge is inside the pixel, and for this, the maximum distance from the center of the pixel may be $1/\sqrt{2}$.}, allowing the polynomial to become zero in a neighborhood 
of $\rho=0$ ($\rho\leq\rho_0$). This parameter must be greater than zero and and less than $\frac{1}{\sqrt{2}}$. Its optimal 
