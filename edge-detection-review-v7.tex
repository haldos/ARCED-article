%-------------------------------------------------------------------------------
% A Review of Classic Edge Detectors - v7
...
The first family of algorithms reviews the use of first derivative to find the changes of intensity, 
...
Results of the implemented algorithms are presented in section \ref{sec:results}, along some examples 
...
Having symmetrical edges is a desirable property and can only be achieved with oddly sized mask, the smallest 
...
$[-1\ 0\ 1]$ with vertically moving average $[1\ 1\ 1]$, and vice versa. Hence, these operators have 
...
Sobel masks can be obtained (beyond normalization) as the convolution of a horizontal derivation mask 
$[-1\ 0\ 1]$ with vertically filter $[1\ 2\ 1]$ (closer to a Gaussian response than Prewitt), and vice versa. 
Hence, these operators have smoothing properties, as mentioned in the preceding paragraph. \\

Computational difference between Prewitt and Sobel masks is not an issue. It is preferable to 
use Sobel operators, because they are better localized, and their filter are also less aliased, 
because of the Gaussian shape of $[1\ 2\ 1]$.
...
is obtained, containing the approximation of horizontal and vertical components of the first derivative, 
$g_x$ and $g_y$ respectively. Gradient magnitude image $M$ is calculated using the equation \ref{eq:mag_alpha} 
(Figure \ref{fig:profiles3d-c}). \\

Finally, edges image is obteined by thresholding the gradient magnitude image, i.e.\ for each pixel in the image compute
\begin{equation*}
...
should be used (e.g.\ marching squares \cite{marching_cubes}).
...
This section explains in detail the original work by Haralick, particularly is applied the same 
technique bicubic polynomial fitting (see \ref{sec:bicubic}). There are other options for image 
interpolation \cite{getreuer}. \\
...
By substituting the 25 data points into the polinomial equation (equation \ref{eq:bicubic}), the following system of 
...
polynomial are taken, the gradient angle $\theta$, defined with positive y-axis, is defined as
...
have to be maded, one with the kernel and another with the Laplacian operator).\\
...
\end{center}
Required external libraries: \texttt{iio} (\url{http://dev.ipol.im/git/coco/iio.git}).
...
\begin{figure}[t!]
	...
	\quad
	\subfigure[Grayscale image.]{\includegraphics[width=0.48\textwidth]{results/molino_bw.png}}
