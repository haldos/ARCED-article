...
\href{http://dev.ipol.im/~juanc/ipol_demo/sc_edges/}{online demonstration}\footnote{\url{http://dev.ipol.im/~juanc/ipol_demo/sc_edges/}} is also available, where 
...
In 1980 Marr and Hildreth \cite{segm:edge_region:marr:80:digital_step} argued that intensity changes are not independent 
...
In addition, we will focus on low-level edge detection algorithms, which means that they will output a disconnected an unordered set of edgels (individual pixel facets where boundaries occur). Algorithms that perform edge integration into objects boundaries are of higher level of abstraction and not considered in this work.
...
\myn{\label{second:references}}The two algorithms presented in this section (Marr-Hildreth \cite{segm:edge_region:marr:80:digital_step} and Haralick \cite{bb20239}) 
...
In Figure \ref{fig:results:parameters:roberts:saliency} we show a summary of the threshold's behavior for this algorithm, in the whole range of possible values $[0,1]$. For each image, this information is summarized in a saliency map which serves to asses the quality of the hierarchical set of edges obtained.
...
If a contour has a high $\alpha^-$ value it means that it will survive for a long time in the merging process. On the other hand, small values mean that it will be removed at early stages.
...
\figRowMultiscaleHHH{test}{roberts}{0004}
  \caption{Results of the \textit{Roberts} algorithm our sample images. For each example we show the original image on the left and the results corresponding to parameter values of 0.1, 0.2 and 0.3 respectively.}
...
\figRowMultiscaleHH{test}{0005}
  \caption{Results of the first derivative algorithms our sample images. For each example we show the original image on the left and the saliency map of each algorithm (Roberts, Sobel, Prewitt) respectively.
...
\figRowMultiscaleHH{test}{0008}
  \caption{Results of the first derivative algorithms our sample images. For each example we show the original image on the left and a saliency map of each algorithm (Roberts, Sobel, Prewitt) respectively.
  The value of each contour corresponds to the scale of disappearance  $\alpha^-$ of each region.}
...
\figRowMultiscaleHHHH{test}{0005}
  \caption{Results of the second derivative algorithms our sample images. For each example we show the original image on the left and a saliency map of each algorithm (Marr-Hildreth-Gaussian, Marr-Hildreth-Log, Haralick) respectively.
  The value of each contour corresponds to the scale of disappearance  $\alpha^-$ of each region.}
...
\figRowMultiscaleHHHH{test}{0008}
  \caption{Results of the second derivative algorithms our sample images. For each example we show the original image on the left and the saliency map of each algorithm (Marr-Hildreth-Gaussian, Marr-Hildreth-Log, Haralick) respectively.
  The value of each contour corresponds to the scale of disappearance  $\alpha^-$ of each region.}
...
\myn{\label{results:discussion}}In this section we will discuss the obtained results and contrast them with the claims made by the original authors.
...
With respect to Prewitt and Sobel Operators, 
...
The obtained results validate the original claims made by Marr \cite{segm:edge_region:marr:80:digital_step}, were he made the following observations:
\begin{itemize}
...
The first claim is validated in our experiments varying the $\sigma$ parameter, where we observed that no single value will allow us to obtain all the meaningful edges. The same reasoning applies to the zero crossing threshold $t_{ZC}$ .
...
All algorithms have their advantages and disadvantages. Choosing one or the other may depend on requirements of the application. Furthermore, we must remark that we focused on low level edge detectors, and thus none of the implemented methods ensure edge connectivity. This algorithms only serve as the input for a further edge integration step. There are many proposals in the literature aiming to integrate edgels into meaningful object boundaries. Examples of these approaches are the Canny \cite{Canny1986} edge detector or the meanignful level lines algorithm \cite{segm:cao:03:extracting_meaningful}.

With respect to the parameters, these methods require the manual determination of the parameters, and it is very hard to find a single threshold value suitable for a broad category of examples. In this sense, \emph{A contrario} methods provide a better approach for setting parameters.
...
This idea can be even extended to pick edges at different scales depending on the region of the image (see \cite{segm:cardelino:2012:a_contrario_selection_of_optimal_partitions} for a similar idea applied to regions).

\subsection{Future work}

The goal of this work was to provide reference implementation for some of the most classical edge detectors. Some of them have been published for many years and are quite simple, however, it is surprisingly hard to find trustable implementations. We focused on low-level edge detection, which means that we do not consider post processing steps to integrate the detected edgels into meaningul lines.
...
A suitable framework for this quantitative analysis is discussed and applied in \cite{segm:cardelino:2012:a_contrario_selection_of_optimal_partitions}, but this is clearly outside the scope of this work.
